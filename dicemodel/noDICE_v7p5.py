...
class PyDICE(object):
    ...
    def __call__(self,
       ...
       # Initial pure rate of time preference for consumption(0.015)
        ...
        if self.vd_switch == 0:
            self.sdr_dam[0] = 0.00000000001
            self.damage_sdf[0] = 0.00000000001
            self.inst_disutil_dam[0] = 0.00000000001
            self.disc_disutil_dam[0] = 0.00000000001
        else:
        ...
        for t in range(1, self.steps):
            ...
            * (self.max_pop/self.pop[t-1])**self.pop_gr)
            ...
            # Endogenous social discount rate for disutility of damage
            if self.vd_switch == 0:
                self.sdr_dam[t] = 0.00000000001
                self.damage_sdf[t] = 0.00000000001
                self.inst_disutil_dam[t] = 0.00000000001
                self.disc_disutil_dam[t] = 0.00000000001
            else:
           ...
           # Period Welfare term
            ...
            self.scc[t] = ((self.tstep * self.scale1 * (-1000 * self.e[t] / (self.c[t]))) + self.scale2)

            # self.atfrac[t] = ((self.mat[t]-588)/(self.ccatot[t]+.000001))

            # self.atfrac2010[t] = ((self.mat[t]-self.mat[0]) / (.00001+self.ccatot[t]-self.ccatot[0]))
        ...
        self.data = {
                     ...
                     'Population': self.pop,
                     'Utility of Consumption': self.welfare_utility,
                     'Disutility of Damage': self.welfare_disutility,
                     'Undiscounted Utility of Consumption': self.inst_util_con,
                     'Undiscounted Disutility of Damage': self.inst_disutil_dam,
                    ...
                    #  'Undiscounted Period Welfare': self.undiscounted_welfare,
                     ...
                     'Utility of Consumption 2300': self.welfare_utility[60],
                     'Disutility of Damage 2300': self.welfare_disutility[60],
                     'Undiscounted Utility of Consumption': self.inst_util_con[60],
                     'Undiscounted Disutility of Damage': self.inst_disutil_dam[60],
