...
class PyDICE(object):
    ...
    def __call__(self,
                 ...
                 t2xco2_dist=0,
                 tfp_gr=0.079,
                 sigma_gr=-0.01, # growth rate sigma = CO2-equivalent-emissions output ratio
                 pop_gr=0.134,
                 fosslim=6000,
                 cback=344,
                 decl_back_gr=0.025,
 ...
 # Savings rate (optlrsav = 0.2582781457) from the control file
        ...
        self.disc_util_con[0] = self.inst_util_con[0] * self.pop[0] * 1000

        ## Disutility of damage

        # Social discount rate of damage
        self.sdr_dam[0] = 0.00
        self.damage_sdf[0] = 1.00

        # Instantaneous disutility of damage
        if self.emdd == 1.00:
            self.inst_disutil_dam[0] = np.log(self.damages[0] * 1000 / self.pop[0])
        else:
            self.inst_disutil_dam[0] = (((self.damages[0]*1000/self.pop[0]) ** (1.0 - self.emdd) - 1.0)/ (1.0 - self.emdd) - 1.0)

        # Discounted disutility of damage
        self.disc_disutil_dam[0] = self.inst_disutil_dam[0] * self.pop[0] * 1000
        ...
        for t in range(1, self.steps):
            ...
            self.consumption_sdf[t] = 1.00/((1.00 + self.sdr_con[0])**self.tstep*(t))
            ...
            # Period disutility
            if self.emdd == 1.00:
                self.inst_disutil_dam[t] = np.log(self.damages[t]* 1000/self.pop[t])
            else:
                self.inst_disutil_dam[t] = (((self.damages[t] *1000/self.pop[t]) ** (1.0 - self.emdd) - 1.0)/ (1.0 - self.emdd) - 1.0)
