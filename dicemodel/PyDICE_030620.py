...
# shridhar: self.s defined here = savings rate, or it is actually just savings values, even though it is called self.sr. look at initialization of econ parameters, investment and savings: self.s[0] = self.sr
    ...
    #shridhar: savings rate is specified here, this determines investments & therefore savings
        ...
        # Damage intercept #shridhar: do I need to change this?
        self.a1 = 0.0
        ...
        #shridhar: EMUC = 1.45 defined here
        self.emuc = 1.45

        # Elasticity of marginal utility of damage (1.000001)
        #shridhar: re-define initial value
        self.emdd = 1.000001
        ...
        for t in range(1, self.steps):
            ...
            # Gross world product (net of abatement and damages)
#askshajee: where is damage in the function?
            # self.y[t] = self.ynet[t]-self.abatecost[t]        #shajee's function
            self.y[t] = self.ynet[t] -self.abatecost[t] -self.damages[t]  #my function
...
#askshajee: why is there a limit to output without affecting any production inputs? I'm taking this out to capture actual growth effects
            ...
            optlrsav = ((self.dk + 0.004)
                        / (self.dk + 0.004 * self.emuc + self.sdr_con)
                        * self.gama)
            ...
            # consumption per capita
        #askshajee: why is the per capita consumption multiplied by 1000? There is no division by 1000 later when utility is calculated
            self.cpc[t] = self.c[t]*1000/self.pop[t]
        #shridhar: why are there separate lower limits to output, consumption and also per capita consumption? removing this too
        ...
        self.sdr_con[t] = (prtp_con + (self.emuc * self.con_g[t]))
        self.consumption_sdf[t] = 1.00/((1.00 + self.sdr_con[t])**self.tstep)

        #askshajee: I think brackets are wrong here
            # self.util_sdr[t] = 1/((1+self.irstp)**(self.tstep*(t))) 
        
           
        # Instantaneous utility of consumption
        #Shridhar: I'm changing this from U(C)= C^((1-emuc)-1)/((1-emuc)-1). check func visualization, this is wrong
        self.inst_util_con[t] = (((self.cpc[t])** (1.0 - self.emuc))/(1.0 - self.emuc))
    

        # discounted utility, normalized for population (from per capita)
        self.disc_util_con[t] = self.inst_util_con[t] * self.pop[t] * self.consumption_sdf[t]

        ### Disutility of Damage

        # social discount rate and factor for disutility of damage
        ...
        # discounted disutility, normalized for population (from per capita)
        self.disc_disutil_dam[t] = self.inst_disutil_dam[t] * self.pop[t] * self.damage_sdf[t]

        # Period Welfare function (scaled)
        self.welfare[t] = self.welfare[t-1] + ((self.tstep * self.scale1 * np.sum(self.disc_util_con -self.disc_disutil_dam)) + self.scale2)
